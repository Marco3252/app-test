...
.buscador-fijo { 
  ...
  z-index: 1; 
  /* mayor que el contenido, menor que tu menú si este es fijo */ 
...
<style>
  .buscador-fijo form { margin: 0 20px; /* margen solo a los lados */ }
   .buscador-fijo {
    ...
    .buscador-fijo label {
      font-size: 1.3rem; /* label más grande */
      font-weight: bold;
      margin-bottom: 15px;
      display: block;
      text-align: center;
    ...
    .buscador-fijo i {
      font-size: 22px;
      color: #333;
      margin-right: 10px;  /* separación respecto al input */
    }
...
function cambiarImagenSiExiste(url) { 
  ...
  imgTest.onload = function() { 
    // Si la imagen se cargó bien, reemplaza el src 
...
.then(data => {
  if (data.success) {
    //console.log('📚 Cursos del alumno:', data.data);
    // data.data[0] será el curso activo (si existe)
...
function updateCourseText(){
  courseText.textContent=cursos.length
    ? `Este alumno está ingresando al curso: "${cursos[0].nombre}"`
...
function registrarAsistencia(alumnoId, cursoId, force = false) {
  ...
  .then(data => {
    ...
    if (data.needs_confirm) {
      if (confirm("Ya se registró la salida ¿Desea sobreescribirla?")) {
